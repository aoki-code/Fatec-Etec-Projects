...
select id, ano from filme where titulo = 'Interestelar'
select id, ano from filme where titulo like '____plash'
select id, nome, nome_real from estrela where nome like 'Rosa'
select filme_id, convert(char(10), data_fabricaçao, 103) as fab from dvd where data_fabricaçao = '2020-01-01'
select dvd_num, data_locaçao, data_devoluçao, valor, cast(valor + 2.00) as valor_com_multa from locaçao where cliente_num_cadastro = 5503
select logradouro, num, cep from cliente where nome = 'Matilde Luz'
select nome_real from estrela where nome = 'Michael Keaton'
select num_cadastro, nome, logradouro + ' ' + num + ' ' + cep as end_comp from cliente where num_cadastro = 5503 or num_cadastro > 5503